...
In order to minimize the cost of permanently storing access lists, we suggest the execution clients to maintain
a per-block "combined access list", and replace the entries of individual transactions' `accessList` with a
compact 2 bytes long reference to the "combined access list".

With this approach shared entries in the access lists cannot cause sufficient bloating of the blockchain state size.
There is no need to introduce any observable changes to the protocol as this "compression" can be unwrapped by the
execution clients in real time.
